...
public class Authorityinteceptor implements HandlerInterceptor{
    ...
    public boolean preHandle(HttpServletRequest httpServletRequest, HttpServletResponse httpServletResponse, Object o) throws Exception {
        ...
        if (StringUtils.equals(className,"UserManageController") && StringUtils.equals(methodName,"login")){
            log.info("权限拦截器拦截到请求，calssName:{},methodName:{}",className,methodName);
            //如果是拦截登陆请求，不打印参数，因为参数里面有密码，全部会打印到日志中，防止日志泄露
            return true;
        }

        log.info("权限拦截器拦截到请求，calssName:{},methodName:{},param:{}",className,methodName,requestParamBuffer.toString());
        ...
        if (user == null || (user.getRole().intValue() != Const.Role.ROLE_ADMIN)){
            ...
            httpServletResponse.setContentType("application/json;charset=UTF-8");//这里是设置返回值的类型，因为全部都是json接口。
